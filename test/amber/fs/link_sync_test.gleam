...
pub fn link_sync_success_test() {
  let test_dir = amber.make_temp_dir_sync([])
  ...
  let new_data = amber.read_text_file_sync(new_name)
  ...
  should.equal(new_data2, amber.read_text_file_sync(old_name))
  ...
  should.equal(new_data3, amber.read_text_file_sync(new_name))

  // Remove oldname. File still accessible through newname.
  amber.remove_sync(old_name, [])
  let new_name_stat = amber.stat_sync(new_name)
