import amber/web/binary_type.{type BinaryType}
import amber/web/close_event.{type CloseEvent}
import amber/web/message_event.{type MessageEvent}
import amber/web/web_socket_option.{type WebSocketOption}
...
/// Creates a new `WebSocket` connection to the given URL with options.
///
/// ## Examples
///
/// ```gleam
/// let ws = web_socket.new_with("ws://localhost:8080", [
///   web_socket_option.Protocols(["json"]),
/// ])
/// ```
///
@external(javascript, "./web_socket.ffi.mjs", "new_with")
pub fn new_with(url: String, options: List(WebSocketOption)) -> WebSocket
...
/// in conjunction with the `Protocols` option to perform subprotocol
/// negotiation.
///
