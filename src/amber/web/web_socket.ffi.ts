import type * as $webSocket from "$/amber/amber/web/web_socket.mjs";
import { fromBinaryType, toBinaryType } from "~/amber/web/binary_type.ts";
import { toCloseEvent } from "~/amber/web/close_event.ts";
import { toWebSocketOptions } from "~/amber/web/web_socket_option.ts";
...
export const new_with: typeof $webSocket.new_with = (url, options) => {
  return new WebSocket(url, toWebSocketOptions(toArray(options)));
};
